...
const Person = ({ person }) => {
  ...
  const handleDelete = () => {
    const swalWithBootstrapButtons = Swal.mixin({
      customClass: {
        confirmButton: 'w-full bg-red-700 rounded text-bold text-xl py-4 px-2 text-white m-2 block',
        cancelButton: 'w-full bg-green-700 rounded text-bold text-xl py-4 px-2 text-white m-2 block'
      },
  ...
  return (
    <tr key={person.id}>
      ...
      {person.professional ? <td className="border-2 text-xs px-4 py-2 text-sm">{person.professional.occupation.map(e => e + " ")}</td> : <td className="border-2 text-xs px-4 py-2 text-sm">unemployed</td>}
